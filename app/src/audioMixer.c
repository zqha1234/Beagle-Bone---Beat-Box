...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	 ...
	 */
	bool flag = false; 	// to recor the info if there is an avaiable space for the sound
	...
	if (!flag) {
		printf("Failed to insert the sound!\n");
	...
	pthread_mutex_lock(&audioMutex);
	{
		for (int i = 0; i < MAX_SOUND_BITES; i++) {
			if (!soundBites[i].pSound) {
				...
				int j = 0;
				while (j < size && location < sound->numSamples) {
					int value = sound->pData[location] + buff[j];
					if (value > SHRT_MAX) {
						value = SHRT_MAX;
